...
** Internal Packages
   #+BEGIN_SRC emacs-lisp
     (defvar max/vendor-dir (expand-file-name "vendor" user-emacs-directory))
     (add-to-list 'load-path max/vendor-dir)

     (dolist (project (directory-files max/vendor-dir t "\\w+"))
...
** Line Numbers
   ...
   #+RESULTS:
   | highlight-indentation-mode | nlinum-relative-mode |
